...
//
// IPs come from req.headers["x-forwarded-for"] (Vercel injects this; see
// clientIp below for what a self-hosted proxy must do); callers should use
// clientIp(req).
...
// The left-most X-Forwarded-For entry is the client ONLY if the proxy in
// front of us overwrites the header with the address it saw. Vercel does.
// Off Vercel, Nginx must do the same ($remote_addr after real_ip from
// CF-Connecting-IP, see docs/DEPLOY.md §6): appending with
// $proxy_add_x_forwarded_for keeps whatever the client sent in first place,
// and rotating that value would walk around every per-IP limit, the ones on
// the payment endpoint included. Capped because the value becomes part of a
// rate-limit key (an IPv6 address is at most 45 characters).
const MAX_IP_LEN = 64;

function clientIp(req) {
  const fwd = req.headers?.["x-forwarded-for"];
  if (typeof fwd === "string" && fwd.length > 0) {
    const first = fwd.split(",")[0].trim().slice(0, MAX_IP_LEN);
    if (first) return first;
  }
  const real = req.headers?.["x-real-ip"];
  if (typeof real === "string" && real.trim().length > 0) return real.trim().slice(0, MAX_IP_LEN);
  return "unknown";
}
